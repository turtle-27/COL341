...
def main():
    ...
    #Plotting param
    f_param = open("my_params.txt", "w")
    f_param.write(str(epochs))
    f_param.write(str(batch_size))
    f_param.write(str(learning_rate_type))
    f_param.write(str(learning_rate))
    f_param.write(str(activation_type))
    if(arc_type == 1):
        f_param.write("2")
    else:
        f_param.write("1")
    f_param.write(str(seed_value))
        ...
        for j in range(len(X_batch)):
            if(arc_type == 1):
                if(flag):
                    ...
                    A = forward_propagation(X_batch[j], temp, activation_type, num_hidden, loss_type)
                delta_L = backward_propagation(W,A,Y_batch[j],1, num_hidden, loss_type)
                # break
